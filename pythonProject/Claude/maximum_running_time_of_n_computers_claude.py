...
class Solution:
    def maxRunTime(self, n: int, batteries: List[int]) -> int:
        # If we have fewer batteries than computers, it's impossible
        if len(batteries) < n:
            return 0
            
        # Sort batteries for potential optimization
        batteries.sort()
        
        # Binary search for the maximum running time
        left, right = 1, sum(batteries) // n  # Maximum possible time
        ...
        while left < right:
            ...
            for battery in batteries:
                # Each battery can contribute at most 'mid' power
                available_power += min(battery, mid)
                
                # Early termination if we have enough power
                if available_power >= total_power_needed:
                    break
            
            if available_power >= total_power_needed:
                left = mid  # We can run for at least 'mid' minutes
            else:
                right = mid - 1  # We need to reduce our target
