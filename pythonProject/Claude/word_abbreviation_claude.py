class Solution:
    def wordsAbbreviation(self, words):
        # Helper function to get abbreviation based on prefix length
        def get_abbr(word, prefix_len):
            # Calculate the number of characters in between first prefix_len and last character
            middle_count = len(word) - prefix_len - 1
            
            # If abbreviation doesn't make the word shorter (when middle_count <= 1)
            # or the resulting abbreviation is not shorter than the original word
            if middle_count <= 1 or prefix_len + 1 + len(str(middle_count)) >= len(word):
                return word
            else:
                # First prefix_len characters + number of characters in between + last character
                return word[:prefix_len] + str(middle_count) + word[-1]
        ...
        for i in range(n):
            result[i] = get_abbr(words[i], prefix_lengths[i])
        ...
        while True:
            ...
            for i in range(n):
                ...
                if abbr not in abbr_groups:
                    abbr_groups[abbr] = []
                abbr_groups[abbr].append(i)
            
            # Check if any collisions remain
            has_collision = False
            
            # Resolve collisions by increasing prefix length
            for abbr, indices in abbr_groups.items():
                if len(indices) > 1:  # Collision found
                    has_collision = True
                    for i in indices:
                        prefix_lengths[i] += 1
                        result[i] = get_abbr(words[i], prefix_lengths[i])
            
            # If no collisions are found, we're done
