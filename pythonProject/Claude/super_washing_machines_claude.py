class Solution(object):
    def findMinMoves(self, machines):
        ...
        # Track the balance of dresses at each step
        balance = 0
        result = 0
        
        for machine in machines:
            # Calculate how many dresses this machine needs to give/receive
            machine_difference = machine - target
            
            # Update the running balance (negative means overall deficit to the left)
            balance += machine_difference
            
            # The minimum moves is determined by:
            # 1. Max number of dresses that need to move from a single machine
            # 2. Max accumulated imbalance at any point
            result = max(result, abs(balance), machine_difference)
