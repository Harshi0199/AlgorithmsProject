class Solution(object):
    def minCost(self, nums, cost):
        ...
        :rtype: int
        """
        # Create a list of (num, cost) pairs and sort by num
        pairs = sorted(zip(nums, cost))
        
        # Calculate the total cost
        total_cost = sum(cost)
        
        # Find the weighted median - the element where we have half the total cost
        # on each side
        prefix_sum = 0
        for num, c in pairs:
            prefix_sum += c
            # Once we cross the halfway point, we've found our target
            if prefix_sum >= total_cost / 2:
                target = num
                break
