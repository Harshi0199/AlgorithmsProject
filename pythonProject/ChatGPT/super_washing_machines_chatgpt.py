class Solution(object):
    def findMinMoves(self, machines):
        ...
        # Check if it is possible to evenly distribute dresses
        if total % n != 0:
            return -1
        ...
        for dresses in machines:
            # How much this machine is above or below the target
            diff = dresses - target
            cumulative_balance += diff
            # The maximum moves at any point is the maximum of:
            # - absolute value of cumulative balance (to balance load across multiple machines)
            # - individual machine's surplus (as it may need that many moves to transfer out)
